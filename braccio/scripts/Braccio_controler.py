...
#Pilotage du robot
def pilotage():
    moteur_droit  = int16(0)
    moteur_gauche = int16(0)
    controlOeuf   = uint8(0)
    ...
    while not close and not fin_pilotage:
        ...
        moteur_droit  = int16(-255*joystick.get_axis(R_joy_y))
        moteur_gauche = int16(-255*joystick.get_axis(L_joy_y))

        if moteur_droit  < 16 and moteur_droit  > -16:
            moteur_droit  = int16(0)
        if moteur_gauche < 16 and moteur_gauche > -16:
            moteur_gauche = int16(0)
        ...
        textPrint.tprint(screen, "Moteur gauche : {}".format(moteur_gauche))
        textPrint.tprint(screen, "Moteur droit  : {}".format(moteur_droit))
        ...
        if len(mouvements) != 0:            
            for k in range(0,len(mouvements)):
                if selection == k:
                    textPrint.indent()
                    if jouer_mouvement:
                        textPrint.tprint(screen, mouvements[k] + " <-- en cours")
                    else:
        ...
        #"""#debug
        msg_to_send = [uint8(1),int16(moteur_gauche),int16(moteur_droit),controlOeuf,joueSequence] # uint8(1) is to tell Braccio it is a command order
        braccio.send(bytes(msg_to_send)) 
        #"""#debug
